...
function survivalModifier(status: string): number {
  if (status === "DEAD") {
    return 0.4;
  }
...
function performanceModifier(contribution: number): number {
  return 1 + Math.min(10, Math.max(0, contribution)) * 0.03;
}

function squadMultiplier(mode: string, squadSize: number): number {
  if (mode === "SOLO") {
    return 1.6;
  }
...
export async function awardEventRewards(prisma: PrismaClient, eventId: number): Promise<string[]> {
  ...
  for (const participant of event.participants) {
    ...
    const multiplier =
      squadMultiplier(participant.mode, squadSize) *
    ...
    summaries.push(
      `${participant.character.name}: +${xp} XP, +${coins} монет, +${eventPoints} очков, +${iceShards} оск.`
    );
  }
