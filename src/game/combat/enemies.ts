...
export type EnemyConfig = {
  ...
  hp: number;
  attackDamage: number;
  vulnerableDomains: GameDomain[];
  resistantDomains: GameDomain[];
  special: string;
  rewardWeight: number;
};
  ...
  phases: [
    {
      ...
      vulnerableDomains: ["magic"] as GameDomain[],
      resistantDomains: ["strength"] as GameDomain[]
    },
    {
      name: "Звериный голод",
      vulnerableDomains: ["strength"] as GameDomain[],
      resistantDomains: ["precision"] as GameDomain[]
    },
    {
      name: "Сердце льда",
      vulnerableDomains: ["precision"] as GameDomain[],
      resistantDomains: ["magic"] as GameDomain[]
    }
  ]
};

export function getEnemyConfig(enemyKey: string): EnemyConfig {
