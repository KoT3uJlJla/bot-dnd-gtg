...
): Promise<{ id: number; telegramChatId: bigint; title: string | null }> {
...
export async function getActiveEventByChat(prisma: PrismaClient, chatId: bigint) {
...
export async function startIceWastelandEvent(
  prisma: PrismaClient,
  input: GroupChatInput & { startedByTelegramId: bigint }
...
export async function stopActiveEvent(
  prisma: PrismaClient,
  chatId: bigint
): Promise<ServiceResult> {
...
export async function joinActiveEvent(
  prisma: PrismaClient,
  input: {
    chatId: bigint;
    userId: number;
...
export async function leaveActiveEvent(
  prisma: PrismaClient,
  input: {
    chatId: bigint;
    userId: number;
...
export async function setSoloMode(
  prisma: PrismaClient,
  input: {
    chatId: bigint;
    userId: number;
  ...
  return {
    ok: true,
    message:
      "Ты идёшь один. Награда выше, но риск смерти максимальный."
  };
}

export async function joinOpenSquad(
  prisma: PrismaClient,
  input: {
    chatId: bigint;
    userId: number;
...
export async function eventStatusText(prisma: PrismaClient, chatId: bigint): Promise<string> {
  const event = await getActiveEventByChat(prisma, chatId);
  if (!event) {
...
export async function performSceneAction(
  bot: Bot,
  prisma: PrismaClient,
  input: {
    chatId: bigint;
...
export async function performDeathSave(
  bot: Bot,
  prisma: PrismaClient,
  input: {
    chatId: bigint;
  ...
  const roll = await rollTelegramD6(
    bot,
    prisma,
    event ? { ...rollInput, eventId: event.id } : rollInput
  );
...
async function performChallenge(
  ...
  input: {
    ...
    sceneKey: string;
    threshold: number;
    challengeDomain?: "strength" | "precision" | "magic";
...
async function performAttack(
  ...
  const counterDamage =
    roll.effectiveValue <= 3
      ? input.failureDamage + (enemy.key === "frozen_bully" && nextEnemyHp <= enemy.hp / 2 ? 1 : 0)
      : 0;
  const counterMessage =
    counterDamage > 0 ? await applyIncomingDamage(prisma, input.character, counterDamage) : null;
...
async function ensureCombatSession(
  prisma: PrismaClient,
  eventId: number,
  enemyKey: string
): Promise<{
...
async function advanceScene(
  ...
  if (!nextScene) {
    ...
    await bot.api.sendMessage(
      telegramChatId(chatId),
      `🏆 Акт 1 завершен. Вход к ледяной пещере открыт.\n${rewardText}`
    );
  ...
  await bot.api.sendMessage(
    telegramChatId(chatId),
    `➡️ Следующая сцена: ${nextScene.title}\n${nextScene.publicText}`
  );

  return `Открыта сцена: ${nextScene.title}.`;
}

function progressFromRoll(value: number): number {
  if (value <= 2) {
    return 0;
  }
...
async function calculateSmallBonus(
  prisma: PrismaClient,
  character: Character,
  domain?: "strength" | "precision" | "magic"
): Promise<number> {
  if (!domain) {
    return 0;
  }
...
async function applyIncomingDamage(
  prisma: PrismaClient,
  character: Character,
  damage: number
): Promise<string> {
