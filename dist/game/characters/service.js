...
export async function createCharacter(prisma, input) {
...
export async function getActiveCharacter(prisma, userId) {
...
export async function changeCharacterClass(prisma, characterId, nextClassKey) {
    ...
    if (!character.freeClassChangeAvailable && !rebirthScroll) {
        return {
            ok: false,
            reason: "Бесплатная смена класса уже использована. Нужен Свиток Перерождения."
        };
    }
...
export async function grantStarterLoadout(prisma, userId, characterId) {
...
export async function consumeInventoryItem(prisma, characterId, itemKey) {
...
export function characterProfileText(character) {
...
export function classesHelpText() {
    return GAME_CLASSES.map((item) => `${item.name} — ${item.role}. Домен: ${item.domain}. HP/Mana/Stamina: ${item.hpMax}/${item.manaMax}/${item.staminaMax}.`).join("\n");
}
