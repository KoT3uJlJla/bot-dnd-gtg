...
import { changeCharacterClass, characterProfileText, classesHelpText, createCharacter, getActiveCharacter } from "../../game/characters/service.js";
...
import { markPrivateStarted, requirePrivateAccess, upsertUserFromContext } from "../middleware/access.js";
const pendingCharacterNames = new Map();
function commandArg(match) {
    return typeof match === "string" ? match.trim() : "";
}
export function registerPrivateCommands(bot) {
    ...
    bot.command("equipment", async (ctx) => {
        ...
        await ctx.reply(equipment.length > 0
    ...
    bot.command("use", async (ctx) => {
        ...
        const update = config.consumableEffect === "heal_hp"
    ...
    bot.callbackQuery(/^change_class:(.+)$/, async (ctx) => {
        ...
        await ctx.answerCallbackQuery(result.usedRebirthScroll ? "Класс изменен через Свиток Перерождения." : "Класс изменен.");
